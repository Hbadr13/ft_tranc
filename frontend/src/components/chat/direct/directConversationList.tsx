...
export default function DirectConversationList({ currentUser, msg2, users, amis, setStatus_Tow_User, status_tow_user }: { currentUser: userProps, msg2: string, users: userProps[], amis: userProps[], setStatus_Tow_User: (value: boolean) => void, status_tow_user: boolean }) {
    ...
    return (
        <div className=' w-full h-full flex justify-start items-center flex-col'>
            {
                !click ? (
                    <button onClick={() => setClick(true)} className=" -mt-4 text-white  bg-blue-400 rounded-full justify-center w-[90%] h-10 items-center  duration-300 hover:scale-105">
                    ...
                    click ? (
                        <div className=" bg-slatfe-400 fbg-sky-300  w-full h-full flex  flex-col ">
...

                            {
                                ...
                                )) : <div className='mt-10 w-full justify-center  flex'>List des amis is empty</div>
                            }
                        </div>
                    ) : (
                        ...
                        <div className="  flex  flex-col items-center justify-center">
                            {(conversationList.length != 0) ? conversationList.map((item: listConversationDirect, index) => (
                                <button key={index} onClick={() => { router.replace(`/chat?user=${item.id}`) }} className={`h-20 mt-3 w-full md:p-2 ${item.id == receiver.id ? 'md:bg-blue-200 md:shadow-lg md:shadowf-black ' : 'md:bg-white md:hover:shadow-lg md:hover:bg-blue-200 '}  justify-between items-center inline-flex   ${receiver.id ? 'md:border-2 border-sky-400 ' : 'border-2 border-sky-400 '}   rounded-xl`}>
