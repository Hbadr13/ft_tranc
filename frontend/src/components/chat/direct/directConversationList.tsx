                                    ...
                                    <button key={index} onClick={() => { router.replace(`/chat?user=${item.id}`) }} className={`h-20 mt-3 w-full md:p-2 ${item.id == receiver.id ? 'md:bg-blue-300 md:shadow-lg md:shadowf-black ' : 'md:bg-white md:hover:shadow-lg md:hover:bg-sky-100 '}  justify-between items-center inline-flex    ${receiver.id ? 'md:border' : 'border'}  border-sky-500   duration-1000  transition shahydow-md rounded-xl`}>
                                        <div className="h-auto  justify-start items-center gap-2.5 flex">
                                            ...
                                            <div className="   flex flex-col justify-center items-start space-y-1 ">
                                                <h4 className={` ${receiver.id ? 'hidden' : null}   md:flex text-lg md:text-md`} >{item.username}</h4>
                                            </div>
                                        </div>
                                    </button>
                                )) : null
                            ...
                            {(conversationList.length != 0) ? conversationList.map((item: listConversationDirect, index) => (
                                <button key={index} onClick={() => { router.replace(`/chat?user=${item.id}`) }} className={`h-20 mt-3 w-full md:p-2 ${item.id == receiver.id ? 'md:bg-blue-300 md:shadow-lg md:shadowf-black ' : 'md:bg-white md:hover:shadow-lg md:hover:bg-sky-100 '}  justify-between items-center inline-flex   ${receiver.id ? 'md:border' : 'border'}   border-sky-500   duration-1000  transition shahydow-md rounded-xl`}>
                                    <div className={`h-auto  w-full bg-blsack ${receiver.id ? 'justify-center' : null}  space-x-2 md:justify-start items-center gasp-2.5 flex`}>
                                        ...
                                        {/* {!item.flag && <img className="w-16 h-16 rounded-full" src="https://cdn3.iconfinder.com/data/icons/shape-icons/128/icon48pt_different_account-512.png" />} */}
                                        {!item.flag && <img className={`w-16 h-16 ${receiver.id ? `${item.id == receiver.id ? ' md:border-0 border-4 border-sky-500  ' : ' md:border-0 border-4 border-white '}` : null}   shadow-md shadodw-black md:w-16 md:h-16 rounded-full`} src={item.foto_user} />}
                                        {item.flag && <img className="w-16 h-16 rounded-full" src="https://cdn3.iconfinder.com/data/icons/shape-icons/128/icon48pt_different_account-512.png" />}
                                        <div className="   flex flex-col justify-center items-start space-y-1 ">
                                            <h4 className={` ${receiver.id ? 'hidden' : null}   md:flex text-lg md:text-md`} >{item.username}</h4>
                                        </div>
