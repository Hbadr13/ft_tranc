    ...
    useEffect(() => {
        chatSocket?.on('updateConv', async () => {
            ...
            try {
                ...
                const content = await response.json();
                setMyStatusInRoom1(content)
                console.log('->>gol ach kayn>', myStatusInRoom1);
            } catch (error) {
                ...
                ((receiver.id != 0 || Room.id != 0)) ? (
                    <>
                        <div className=' w-full flex justify-center items-center '>
                            ...
                            <div className=' flex w-[97%] bg-white relative h-16 rounded-xl border-2 justify-start items-center  border-sky-400 wml-3 mt-2 '>
                                {button == false && <div className=" bg-sflate-400  sm:ml-4 w-auto flex" >
                                    ...
                                    <div className="flex-col w-full bg-fslate-500 space-x-2  ml-2">
                                        <h1 className="text-xl font-semibold mt-1">{receiver?.username}</h1>
                                        <div className="">
                                            <div className='flex space-x-1 fbg-black mr-4 w-full'>
                                ...
                                {!button && <div className='absolute   flex items-center right-10 md:right-2  h-full '>
                                    <button onClick={() => {
                                        console.log('play with friend', receiver?.id, onlineUsersss.includes(receiver?.id))
                        ...
                        <div className={` w-full  break-all   h-12 z-h10 mt-2  ${flag == true ? 'blur-sm  lg:blur-0' : null} lg:${flag == true ? null : null} flex flex-row px-3`}>
                            {
                                (status.status == "accepted" || !status) && (!myStatusInRoom1.timeMute && !myStatusInRoom1.isBanned) &&
                                <>
                                    <input
                                    ...
                                    status.status != "accepted" && <>
                                        {
                                            ...
                                            <div className=' w-full h-full  flex justify-center items-center'>
                                                <button onClick={() => unblockChatTwoUser()} className='w-[80%] h-full rounded-full flex hover:scale-105 duration-300  items-center  justify-center   text-md  bg-blue-400 shadow-md text-white border-2 mdl-20 border-white'>Unblocked {receiver.username} </button>
                                            </div>
                                        }
                                        {
