...
export default function EditChannel({ Room }: { Room: channelProps }) {
    ...
    const handlParticipants = async (item: string) => {
        ...
        if (item == 'admin') {
            await fetch(`http://localhost:3333/chat/setAdmin/${Room.id}/${participant?.id}`, {
                method: 'POST',
                // credentials: 'include',
            })
        }
    ...
    return (
        <div className='flex  flex-col  w-[20%] h-[820px] border  items-center  mt-12 border-sky-500 rounded-[30px]  '>
            ...
            {
                click == false &&
                <div className=' bg-white h-62 w-[87%]  rounded-lg flex justify-center items-start  shadow-md sdhadow -mt-[630px] z-30'>
                    {
                        <button onClick={() => handlParticipants('admin')}>set this user is admin</button>
                    }
